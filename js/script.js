...
xhr_zones.open('GET', 'http://louisville.lojic.opendata.arcgis.com/datasets/638546493a6f41c098c060eedda851af_0.geojson');
xhr_zones.send();
...
function createZoningList(data) {
    ...
    for (var i = 0; i< zones.features.length; i++) {
        ...
        array.push({
            "address": zoneAddress.toLowerCase(),
...
function createHomesList(homes){
    var homeAddress;
    for (var i = 0; i < homes.length; i++) {
        ...
        homeArray.push(homeAddress.toLowerCase());
    }
}

function main (zones) {
    createHomesList(homes);
    zoneArray = createZoningList(zones);
    var housingList = "<table>";
    housingList += "<tr><th>MLS ID#</th><th>Address</th><th>Zone</th></tr>"
    for(var i = 0; i < homeArray.length; i++){
        for(var x = 0; x < zoneArray.length; x++){
            if(homeArray[i] === zoneArray[x].address){
               // if(!zoneArray[x].zone.match(/R./)){
                    housingList += "<tr><td>" + homes[i].list_number + "</td><td>" + zoneArray[x].address + "</td><td>" + zoneArray[x].zone + "</td></tr>";
                //}
            }
        }
    }
    housingList += "</table>";
    document.getElementById('ajax').innerHTML = housingList;
}